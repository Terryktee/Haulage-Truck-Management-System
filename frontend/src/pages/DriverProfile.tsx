...
export default function DriverProfilePage() {
  ...
  const { getDriverById, getJobsForDriver, getActiveJobsForDriver, deleteDriver, getTruckById } = useFleet();
  ...
  if (!driver) return (
    <div className="flex flex-col items-center justify-center py-20">
      ...
      <Button variant="outline" className="mt-4" onClick={() => navigate("/drivers")}>Back to Drivers</Button>
    </div>
  );
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          <Button variant="ghost" size="icon" onClick={() => navigate("/drivers")}><ArrowLeft className="h-4 w-4" /></Button>
          <div>
            <h1 className="font-display text-2xl font-bold tracking-tight">{driver.name}</h1>
            <p className="text-sm text-muted-foreground">{driver.id} · {driver.licenseNumber}</p>
          </div>
        </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <div className="rounded-xl border bg-card p-4">
          ...
          <p className="mt-1 font-display text-2xl font-bold">{totalDeliveries}</p>
        </div>
      </div>
      ...
      {activeJobs.length > 0 && (
        <div className="rounded-xl border bg-card">
          <div className="border-b px-5 py-4">
            <h3 className="font-display text-base font-semibold">Current Assignment</h3>
          </div>
          {activeJobs.map((job) => {
            const truck = job.truckId ? getTruckById(job.truckId) : null;
            return (
              <div key={job.id} className="px-5 py-3 flex items-center justify-between cursor-pointer hover:bg-secondary/30" onClick={() => navigate(`/jobs/${job.id}`)}>
                <div>
                ...
                <div className="text-right text-sm">
                  ...
                  {truck && <p className="text-xs text-muted-foreground mt-0.5">{truck.registration}</p>}
                </div>
              </div>
            );
          })}
      ...
      <div className="rounded-xl border bg-card">
        ...
        <div className="divide-y">
          {allJobs.length === 0 && <p className="px-5 py-8 text-center text-sm text-muted-foreground">No jobs yet</p>}
          {allJobs.map((job) => (
      ...
      <DriverFormDialog open={editOpen} onOpenChange={setEditOpen} editDriverId={driver.id} />
    </div>
  );
}
