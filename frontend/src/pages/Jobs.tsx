...
const columns: { status: JobStatus; label: string; colorClass: string }[] = [
...
export default function JobsPage() {
  ...
  const uniqueTruckIds = [...new Set(jobs.map((j) => j.truckId).filter(Boolean))] as string[];
  const uniqueDriverIds = [...new Set(jobs.map((j) => j.driverId).filter(Boolean))] as string[];

  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <h1 className="font-display text-2xl font-bold tracking-tight">Jobs & Deliveries</h1>
          <p className="text-sm text-muted-foreground">Track and manage delivery jobs</p>
        </div>
      ...
      <div className="flex flex-wrap gap-3">
        ...
        <Select value={truckFilter} onValueChange={setTruckFilter}>
          <SelectTrigger className="w-[160px]"><SelectValue placeholder="Truck" /></SelectTrigger>
          <SelectContent>
            <SelectItem value="all">All Trucks</SelectItem>
            {uniqueTruckIds.map((id) => { const t = getTruckById(id); return <SelectItem key={id} value={id}>{t?.registration ?? id}</SelectItem>; })}
          </SelectContent>
        </Select>
        <Select value={driverFilter} onValueChange={setDriverFilter}>
          ...
          <SelectContent>
            ...
            {uniqueDriverIds.map((id) => { const d = getDriverById(id); return <SelectItem key={id} value={id}>{d?.name ?? id}</SelectItem>; })}
          </SelectContent>
        </Select>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        {columns.map((col) => {
          ...
          return (
            <div key={col.status} className={`rounded-xl border bg-card border-t-4 ${col.colorClass}`}>
              ...
              <div className="p-2 space-y-2 max-h-[500px] overflow-y-auto">
                {colJobs.length === 0 && (
                  <p className="text-center text-xs text-muted-foreground py-6">No jobs</p>
                )}
                {colJobs.map((job) => {
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center justify-between mb-2">
                        <span className="font-mono text-xs font-medium">{job.id}</span>
                        <Badge className={`text-[10px] ${statusBadgeStyles[job.status]}`}>{job.status}</Badge>
                      </div>
                      ...
                      <div className="mt-2 flex flex-wrap gap-2 text-[11px] text-muted-foreground">
                        {truck && <span className="flex items-center gap-1"><Truck className="h-3 w-3" />{truck.registration}</span>}
                        {driver && <span className="flex items-center gap-1"><User className="h-3 w-3" />{driver.name}</span>}
                      </div>
                    </div>
                  );
                })}
