...
export default function TruckListPage() {
  ...
  const filtered = useMemo(() => {
    return trucks.filter((t) => {
      const matchSearch = !search || t.registration.toLowerCase().includes(search.toLowerCase()) || t.id.toLowerCase().includes(search.toLowerCase());
  ...
  const handleDelete = (id: string) => {
    ...
    if (activeJobs.length > 0) {
      toast({ title: "Cannot delete", description: `Truck has ${activeJobs.length} active job(s). Complete or cancel them first.`, variant: "destructive" });
      return;
    }
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap gap-3">
        <div className="relative flex-1 min-w-[200px] max-w-sm">
          ...
          <Input className="pl-9" placeholder="Search by registration or ID..." value={search} onChange={(e) => { setSearch(e.target.value); setPage(1); }} />
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y">
              {paginated.map((truck) => (
                <tr key={truck.id} className="hover:bg-secondary/30 transition-colors">
                  ...
                  <td className="px-5 py-3">
                    <Badge className={statusStyles[truck.status]}>{statusLabels[truck.status]}</Badge>
                  </td>
                  <td className="px-5 py-3">
                    <div className="flex justify-end gap-1">
                      <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => navigate(`/trucks/${truck.id}`)}>
                        <Eye className="h-4 w-4" />
                      </Button>
                      <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => { setEditId(truck.id); setFormOpen(true); }}>
                        <Pencil className="h-4 w-4" />
                      </Button>
                      <Button variant="ghost" size="icon" className="h-8 w-8 text-destructive hover:text-destructive" onClick={() => handleDelete(truck.id)}>
              ...
              {paginated.length === 0 && (
                <tr><td colSpan={6} className="px-5 py-8 text-center text-muted-foreground">No trucks found</td></tr>
              )}
            </tbody>
          </table>
        </div>
